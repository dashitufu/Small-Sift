...
{//对正定矩阵做Cholosky分解，结果放在啊B A=BxB'
	...
	{
		iPos_kk = k * iOrder + k;
		B[iPos_kk] = sqrt(B[iPos_kk]);	//对角线等于其开方，显而易见
...
{//用移位法求A矩阵近似p的特征值及其特征向量
	double p1 = p + 0.1;	//错开一点才行，否则会有数值错误
	double A1[3][3] = { {A[0][0] - p1, A[0][1],A[0][2] },
			{A[1][0],A[1][1] - p1,A[1][2]},
			{A[2][0],A[2][1],A[2][2] - p1} };	//为A-pI
	Disp((double*)A, "A");
...
{//用旋转向量与位移构成齐次变换矩阵
	...
	{
		_T V1[4] = { 0,0,1,0 };
		Rotation_Vector_2_Matrix(V1, R1);
	}
...
{//这就是个傻逼方法，用来欺骗template
	...
	//Get_Inv_Matrix_Row_Op_1((float*)NULL, (float*)NULL, 0, NULL);
	//Get_Inv_Matrix_Row_Op_1((double*)NULL, (double*)NULL, 0, NULL);

	Get_Inv_Matrix_Row_Op_2((float*)NULL, (float*)NULL, 0, NULL);
	Get_Inv_Matrix_Row_Op_2((double*)NULL, (double*)NULL, 0, NULL);
...
{//对于列文马夸中需要调整Sigma_H矩阵使得可逆，即加一个I矩阵
	...
	for (y = 0; y < oA.m_iRow_Count; y++)
	{
		...
		{//改行全0，得加一个
			...
			oA.m_pRow[y] = oA.m_iCur_Item;
			oA.m_pBuffer[oA.m_iCur_Item++] = { (unsigned int)y,(unsigned int)y,1,0 };
		}
		else
		{//一直推到插入的位置
			poItem = &oA.m_pBuffer[oA.m_pRow[y]];
			...
			{//加到链表头
				...
				oA.m_pBuffer[oA.m_iCur_Item] = { (unsigned int)y,(unsigned int)y,1.f,(int)oA.m_pRow[y] };
				oA.m_pRow[y] = oA.m_iCur_Item++;
			}
			else
			{
				while (1)	// (poItem && poItem->x < y)
				{
					...
					{//插入
						...
						oA.m_pBuffer[oA.m_iCur_Item] = { (unsigned int)y,(unsigned int)y,1.f,poItem->m_iRow_Next };
						poItem->m_iRow_Next = oA.m_iCur_Item++;
						break;
					}
					poItem = &oA.m_pBuffer[poItem->m_iRow_Next];
