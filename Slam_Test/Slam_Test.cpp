...
void Huge_Matrix_Test()
{
    float* pI = (float*)malloc(1000 * 1000 * sizeof(float));
    float* pB = (float*)malloc(1000 * sizeof(float));
    float* pX = (float*)malloc(1000 * sizeof(float));
    int i,iResult;
    //Gen_I_Matrix(pI,1000,1000);
    memset(pI, 0, 1000 * 1000 * sizeof(float));
    for (i = 0; i < 1000; i++)
        pB[i] = i, pI[i * 1000 + 999 - i] = 1;
    //Solve_Linear_Gause(pI, 1000, pB, pX,&iResult);
    unsigned long long tStart = iGet_Tick_Count();
    Get_Inv_Matrix_Row_Op(pI, pI, 1000,&iResult);
    printf("%d\n", iGet_Rank(pI, 1000, 1000));
